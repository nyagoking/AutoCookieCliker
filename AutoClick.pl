...
while ( 1 ) {
	...
	if ( $sel->is_element_present($locator_golden_cookie) && &click($locator_golden_cookie, "goldenCookie") ) {
        $power = $sel->get_eval("window.Game.buffs.Frenzy ? window.Game.buffs.Frenzy.multCpS : 1");
        $power *= $sel->get_eval("window.Game.buffs['Dragon Harvest'] ? window.Game.buffs['Dragon Harvest'].multCpS : 1");
        $power *= $sel->get_eval("window.Game.buffs['Elder frenzy'] ? window.Game.buffs['Elder frenzy'].multCpS : 1");
        $power *= $sel->get_eval("window.Game.buffs.Clot ? window.Game.buffs.Clot.multCpS : 1");
        $power *= $sel->get_eval("window.Game.buffs['cursed finger'] ? window.Game.buffs['cursed finger'].multCpS : 1");
        $power *= $sel->get_eval("window.Game.buffs['Click frenzy'] ? window.Game.buffs['Click frenzy'].multClick : 1");
        $power *= $sel->get_eval("window.Game.buffs.Dragonflight ? window.Game.buffs.Dragonflight.multClick : 1");

        printf("Dragonflight: %s, Click frenzy: %s\n", $sel->get_eval("!(!window.Game.buffs.Dragonflight)"), $sel->get_eval("!(!window.Game.buffs['Click frenzy'])"));
		# &buy_upgrade();
	}

	if ( $sel->get_eval("!window.Game.buffs.Dragonflight && !window.Game.buffs['Click frenzy']") ne 'true' ) {
        printf("*%d\n", $power);
        while ( 1 ) {
			&double_click($locator_big_cookie);
			&double_click($locator_big_cookie);
			&double_click($locator_big_cookie);
			&double_click($locator_big_cookie);
			&double_click($locator_big_cookie);
			...
			&double_click($locator_big_cookie);
			&double_click($locator_big_cookie);
			last if ( $sel->get_eval("!window.Game.buffs.Dragonflight && !window.Game.buffs['Click frenzy']") eq 'true' );
		}
		# &click("id=product13", "Prism");
	}
